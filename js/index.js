...
Carousel.prototype.init_events = function (){
    ...
    $('#record').removeClass('spinning');
    $('#record').removeClass('turn-on');
    $('#record').removeClass('turn-off');
    ...
    if ($('#record').hasClass('spinning')) {
      
      $('#record').addClass('turn-off').delay( 5000 );
      $('#record').removeClass('spinning');
      $('#record').removeClass('turn-on');
      $('#record').removeClass('turn-off');
      
    } else { // vinyl not spinning, music paused
      
      $('#record').addClass('turn-on').delay( 5000 );
      $('#record').addClass('spinning');
    }
...
{	
	if (currentIndex != index) { //Avoid to start over the audio by clicking on the current music and mute the audio
		...
		this.$.music[0].setAttribute('src', 'http://diogomoura.esy.es/wp-content/uploads/music-for-the-oraculo/' + index + '.mp3');
		this.changeMusic();
